...
| 阶段 | 课程序号 | 课程主题 | 核心内容 | 实战目标 |
| :--- | :---: | :--- | :--- | :--- |
| **一、工程基础** | 第1课 | **深度学习项目入门** | 项目结构设计、PyTorch基础、DataLoader使用 | 搭建模块化的手写数字识别项目，告别脚本化编程 |
| **二、生成基础** | 第2课 | **初识生成模型 VAE** | 自编码器原理、隐变量、重参数化技巧 | 动手实现VAE，完成图像生成与隐空间插值实验 |
| **三、扩散核心** | 第3课 | **DDPM 原理精讲** | 前向扩散(加噪)、反向去噪原理、噪声预测 | 实现扩散过程数学公式，从零构建DDPM训练框架 |
| | 第4课 | **DDPM 代码实战** | U-Net网络架构、时间步编码、采样算法 | 训练DDPM模型，实现从纯噪声生成图像 |
| **四、超分实战** | 第6课 | **初识超分模型** | 超分定义、| 构建超分数据集，实现简单的CNN超分基线模型 |
| | 第7课 | **SR3 模型复现** | 条件扩散模型、SR3网络结构、拼接条件策略 | 修改DDPM代码适配超分任务，训练SR3模型 |
| **五、前沿进阶** | 第8课 | **ResShift 高效超分** | 残差预测思想、跳步采样、加速推理机制 | 解析ResShift源码，体验极速扩散超分效果 |
