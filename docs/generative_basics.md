...
VAE 教你潜变量、概率分布约束、重参数化。扩散模型教你时间步建模、逐步变换、训练目标可以是噪声而不是类别。Flow Matching 教你连续时间视角和速度场预测。两者代表了两种典型的生成建模思路：VAE 更适合讲"潜空间"，DDPM / Flow Matching / ResShift 更适合讲"过程建模"。
...
**Flow Matching 和 DDPM 的本质区别？** DDPM 在离散时间步上预测噪声，Flow Matching 在连续时间上预测速度场。DDPM 的采样是"逐步去噪"，Flow Matching 的采样是"沿速度场积分"。数学上两者都建模从噪声到数据的过程，但视角不同。
